...
<div class="form-section">
    <form method="POST" action="{{ route('admin.content.store') }}">
        ...
        <div class="form-group">
            ...
            <p style="font-size: 0.75rem; color: #6B7280; margin-top: 0.5rem;">مثال: terms-of-service</p>
            @error('slug')
                <p style="color: #DC2626; font-size: 0.875rem; margin-top: 0.5rem;">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <select name="type" class="form-select" required>
                    ...
                    <option value="faq" {{ old('type') == 'faq' ? 'selected' : '' }}>أسئلة شائعة</option>
                    <option value="terms" {{ old('type') == 'terms' ? 'selected' : '' }}>شروط الاستخدام</option>
        ...
        <div class="form-group">
            ...
            <textarea name="content" class="form-textarea" required>{{ old('content') }}</textarea>
            @error('content')
                <p style="color: #DC2626; font-size: 0.875rem; margin-top: 0.5rem;">{{ $message }}</p>
            @enderror
        </div>
